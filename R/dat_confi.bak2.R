...
dat_confi=
  function(n=param(n.confi),m=param(m.confi),d0=param(d0.confi),...) {
    ## TODO: automate calculation of 'm'. see mmix in shell
    ## TODO: decide which cases to keep
    dosim(n=20,m=1e7,d0=d0,id='unif',d.gen=runif,d.args=list(min=d0-3,max=d0+3),...);
    dosim(n=10,m=1e6,d0=d0,id='norm_0.3_0.2',d.gen=rnorm,d.args=list(mean=0.3,sd=0.2),...);
    dosim(n=200,m=1e6,d0=d0,id='norm_0.3_0.2',d.gen=rnorm,d.args=list(mean=0.3,sd=0.2),...);
    dosim(n=10,m=3e6,d0=d0,id='mix_80',d.gen=rnorMix,
          d.args=list(obj=norMix(mu=c(0,0.3),sigma=c(0.05,0.2),w=c(0.8,0.2))),...);
    dosim(n=200,m=7e6,d0=d0,id='mix_80',d.gen=rnorMix,
          d.args=list(obj=norMix(mu=c(0,0.3),sigma=c(0.05,0.2),w=c(0.8,0.2))),...);
    dosim(n=10,m=3e6,d0=d0,id='mix_50',d.gen=rnorMix,
          d.args=list(obj=norMix(mu=c(0,0.3),sigma=c(0.05,0.2),w=c(0.5,0.5))),...);
    dosim(n=200,m=3e6,d0=d0,id='mix_50',d.gen=rnorMix,
          d.args=list(obj=norMix(mu=c(0,0.3),sigma=c(0.05,0.2),w=c(0.5,0.5))),...);
    dosim(n=10,m=3e6,d0=d0,id='mix_20',d.gen=rnorMix,
          d.args=list(obj=norMix(mu=c(0,0.3),sigma=c(0.05,0.2),w=c(0.2,0.8))),...);
    dosim(n=200,m=2e6,d0=d0,id='mix_20',d.gen=rnorMix,
          d.args=list(obj=norMix(mu=c(0,0.3),sigma=c(0.05,0.2),w=c(0.2,0.8))),...);
    ##  cases=expand.grid(n=n,m=m,d0=d0);
   ##  withrows(cases,case, {
   ##    dosim(n=20,m=1e7,d0=d0,id='fig01',d.gen=runif,d.args=list(min=d0-3,max=d0+3),...);
   ##    dosim(n=n,m=1e6,d0=d0,id='norm_0.0_0.2',d.gen=rnorm,d.args=list(mean=0.0,sd=0.2),...);
